...
validating face count, generating 128-dimensional deep feature embeddings,
...
class RegistrationView(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        self.entry_course.pack(fill="x", padx=16, pady=(0, 16))
        ...
        self.lbl_snapshot_img.pack(side="left", padx=10, pady=8)
        ...
        ttk.Separator(left_panel, orient="horizontal").pack(fill="x", padx=16, pady=6)
        ...
        self.btn_toggle_cam.pack(fill="x", padx=16, pady=4)

        ttk.Button(
            left_panel,
            text="📁 Browse Photo File...",
            style="Secondary.TButton",
        ...
        self.status_box.pack(fill="x", padx=16, pady=(16, 0), side="bottom")
        ...
        tk.Label(
            preview_header,
            text="Live Camera Preview & Real-time Face Tracking",
            font=FONT_SUBHEADER,
            fg=config.COLOR_TEXT_PRIMARY,
            bg=config.COLOR_CARD_BG
        ).pack(side="left")
        ...
        self.canvas_cam.create_text(
            320, 240,
            text="Camera feed is stopped.\nClick 'Start Camera' or 'Browse Photo File'",
            fill="#94A3B8",
            font=FONT_BODY,
            justify="center",
            tags="placeholder"
    ...
    def _start_camera_stream(self) -> None:
        ...
        try:
            ...
            self.status_box.config(
                text="Camera could not be accessed.\nYou can use 'Browse Photo File...' to test.",
    ...
    def _poll_frame(self) -> None:
        """Poll latest frame from camera manager and render bounding box overlay."""
    ...
    def _render_frame_to_canvas(self, frame_bgr: np.ndarray) -> None:
        ...
        # Resize preserving aspect ratio to fit canvas
    ...
    def _on_capture_face_clicked(self) -> None:
        """Step 1: Capture a snapshot of the live face and validate detection."""
        if self._current_raw_frame is None:
            messagebox.showerror(
                "Camera Inactive",
                "No live camera feed detected.\nPlease click '▶ Start Camera' or '📁 Browse Photo File...' first.",
                parent=self
            )
            return

        # Run face detector on current live frame
        ...
        if face_count == 0:
            ...
            messagebox.showwarning(
                "No Face Detected",
                "No face detected in the frame.\nPlease ensure your face is well-lit and directly in front of the lens.",
                parent=self
            )
            return
        ...
        if face_count > 1:
            ...
            messagebox.showwarning(
                "Multiple Faces",
                f"Multiple faces ({face_count}) detected in the frame.\nPlease ensure only one student is visible.",
                parent=self
            )
            return

        # Exactly 1 face detected! Freeze snapshot
        target_face = faces[0]
        self._captured_snapshot_frame = frame

        # Crop a nice thumbnail for preview in the left panel
        ...
        if face_crop.size > 0:
            ...
            pil_crop = Image.fromarray(rgb_crop).resize((80, 80), Image.Resampling.LANCZOS)
        ...
        self.lbl_snapshot_status.config(
            text=f"✓ Face Captured & Validated\nConfidence: {target_face.confidence*100:.1f}%\nReady to Register!",
            fg=config.COLOR_SUCCESS,
            font=FONT_BODY_BOLD
        )

        # Enable Register and Retake buttons
    ...
    def _on_retake_clicked(self) -> None:
        """Reset captured snapshot to allow taking a new photo."""
    ...
    def _on_register_clicked(self) -> None:
        """Step 2: Validate inputs and commit registered student using captured snapshot."""
        ...
        if self._captured_snapshot_frame is None:
            messagebox.showwarning(
                ...
                "Please click '📸 Step 1: Capture Face' first to capture the student's face before registering!",
                parent=self
            )
            return

        # Execute registration through service using the validated captured snapshot
    ...
    def _clear_fields(self) -> None:
        """Clear form input fields and reset face snapshot."""
